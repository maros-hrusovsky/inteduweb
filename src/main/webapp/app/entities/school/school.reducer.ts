...
export const ACTION_TYPES = {
  SEARCH_SCHOOLS: 'school/SEARCH_SCHOOLS',
...
export default (state: SchoolState = initialState, action): SchoolState => {
  switch (action.type) {
    case REQUEST(ACTION_TYPES.SEARCH_SCHOOLS):
    ...
    case FAILURE(ACTION_TYPES.SEARCH_SCHOOLS):
    ...
    case SUCCESS(ACTION_TYPES.SEARCH_SCHOOLS):
...
const apiSearchUrl = 'api/_search/schools';

// Actions

export const getSearchEntities: ICrudSearchAction<ISchool> = (query, page, size, sort) => ({
  type: ACTION_TYPES.SEARCH_SCHOOLS,
  payload: axios.get<ISchool>(`${apiSearchUrl}?query=${query}`),
});
